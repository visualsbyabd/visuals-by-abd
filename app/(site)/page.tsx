...
async function getHomeData() {
  try {
    ...
    const [projects, testimonials, services] = await Promise.all([
      // Sort the main-on-home project to the top so it lands in the hero slot,
      // then by order/createdAt for the remaining featured projects.
      Project.find({ status: "published", featured: true })
        .select("title slug category coverImage year client")
        .sort({ isMainOnHome: -1, order: 1, createdAt: -1 })
        .limit(4)
        .lean(),
